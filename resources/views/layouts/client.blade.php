        ...
        " :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
            ...
            <div class="px-4 py-4">
                <div class="flex items-center gap-3 px-3 py-2">
                    <div class="w-10 h-10 bg-gradient-to-br from-warning to-warning/80 rounded-xl flex items-center justify-center text-white font-semibold text-sm shadow-sm">
                        {{ substr(auth()->user()->name ?? 'C', 0, 2) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-text-primary truncate">{{ auth()->user()->name ?? 'Client' }}</p>
                        <p class="text-xs text-text-muted truncate">Client</p>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 py-2 px-3 space-y-1 overflow-y-auto">
                <a href="{{ route('client.dashboard') }}" @click="sidebarOpen = false"
                   class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all border-r-2 {{ request()->routeIs('client.dashboard') ? 'text-primary bg-primary/5 border-r-primary' : 'text-text-secondary hover:text-text-primary hover:bg-surface-container border-r-transparent' }}">
