    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-screen flex flex-col lg:flex-row">
        ...
        " :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
            ...
            <div class="px-4 py-4">
                <div class="flex items-center gap-3 px-3 py-2">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-text-muted truncate">{{ auth()->user()->organization->name ?? 'Staff' }}</p>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 py-2 px-3 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('staff.issues.index') }}" @click="sidebarOpen = false"
                   class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all border-r-2 {{ request()->routeIs('staff.issues*') ? 'text-primary bg-primary/5 border-r-primary' : 'text-text-secondary hover:text-text-primary hover:bg-surface-container border-r-transparent' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
